...
module.exports = function createRecord (req, res) {
	...
	var data = sails.services.utils.replaceNull(actionUtil.parseValues(req));
	
	var user = req.session.user || {};

  _.merge(data, {createdBy : user.id, updatedBy : user.id, userIp:req.ip});
	Model.findOne({id: data.id}).exec(function (err, exist){
		if(exist){
