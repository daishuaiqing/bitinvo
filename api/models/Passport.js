...
function hashPassword (passport, next) {
  ...
  if (passport.password) {
    bcrypt.hash(passport.password, salt, function (err, hash) {
      ...
      sails.models['passport'].find().limit(1)
      .then((data) => {
        if(data.length === 0){
          return next(null, passport);
        }
        let record = Object.assign(data[0], passport);
        let newChksum = sails.services.syncrole.pureSign(record);
        passport.chksum = newChksum;
        sails.log.info(`passport new chksum is ${newChksum}`);
        next(null, passport)
      })
    });
  }
...
var Passport = {
  attributes: {
    ...
    filter: {type: 'boolean', defaultsTo: false},
    ...
    if(passport.SyncFrom){
     sails.models['passport'].find().limit(1)
      .then((data) => {
        ...
        let record = Object.assign({}, obj, passport);
        let newChksum = sails.services.syncrole.pureSign(record);
        passport.chksum = newChksum;
        sails.log.info(`passport new chksum is ${newChksum}`);
        next()
  ...
  beforeUpdate: function (passport, next) {
    if(passport.UpdatedFrom){
      ...
      .then((data) => {
        ...
        let newChksum = sails.services.syncrole.pureSign(record);
        passport.chksum = newChksum;
        sails.log.info(`passport update chksum is ${newChksum}`);
        next()
      })
    }else{
      hashPassword(passport, next);
  ...
  afterCreate: function(item, cb){
    User.findOne({id: item.user})
    .then((data) => {
      if(data && !data.filter){
        sails.services.syncrole.Sync(item, 'passport');
        cb();
      }else{
        cb();
      }
  ...
  afterUpdate: function(item, cb){
    ...
    .then((data) => {
      if(data && !data.filter){
        sails.services.syncrole.Sync(item, 'passport', 'update');
        cb();
      }else{
        cb();
      }
  ...
  afterDestroy: function(item, cb){
    sails.services.syncrole.Sync(item, 'passport', 'destroy');
    cb();
  }
};
