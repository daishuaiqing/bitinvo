...
module.exports = {
  _auth: function (req, res, next) {
    ...
    sails.services.redis.get('onAir', (err, time) => {
      if (err) {
        ...
        res.ok({ info: '开始人脸识别' });
        if (Number(time) === 50) return sails.services.message.local({ topic: 'camera_in_use', value: { time: time, msg: '摄像头正在使用中, 请稍候' } }); // return user:null, if not found
        if (Number(time) > 0) sails.services.message.local({ topic: 'camera_in_use', value: { time: time } });
        setTimeout(() => {
          let message = sails.services.face._verifyUserFace(
            function (user) {
              sails.log.debug(' ##### FaceController:auth:onComplete #### ');
              sails.services.message.local({ topic: 'SGS_MESSAGE_AUTHEN_FACE', value: { user: user, userinfo: getUserInfo } }); // return user:null, if not found
            },
            function (err) {
              sails.log.debug(' ##### FaceController:auth, on error #### ');
              sails.log.error(err);
              sails.services.message.local({ topic: 'SGS_MESSAGE_AUTHEN_FACE', value: { user: null, error: err.error, userinfo: getUserInfo } }); // return user:null, if not found
            },
            function (message) {
            ...
            function () {
              sails.log.debug(`#### FaceController : _auth timeout`);
            }
          );
        }, time * 1000)
      }
  ...
  record: function (req, res, next) {
    ...
    // }
    sails.log.debug(' #### Start Record Face ####');
    let user = req.session.user ? req.session.user : req.user;
    Face.find({ owner: user.id }).then((faces) => {
      if (faces.length > 5) {  //最多拥有5个人脸
                ...
                if (user.id) {
                  ...
                  User.findOne({ id: user.id })
                    .exec(function (err, user) {
                      ...
                      if (user) {
                        Face.create({ owner: user.id, data: message.data }).exec((err, newFace) => {
                          if (err) {
  ...
  recieveSync: function (req, res, next) {
    ...
    Face.findOne({ id: body.id })
      .then((exist) => {
        if (exist) {
          sails.log.info('Face exist');
          return res.ok(exist);
        }
        Face.create(body)
          .then((data) => {
  ...
  clean: function (req, res, next) {
    ...
    if (typeof user !== 'undefined') {
      Face.destroy({ owner: user.id })
        .then((data) => {
          sails.log.verbose(' ##### FaceController:Clean User Face Success #### ');
          return res.ok('Success');
        })
        .catch((err) => {
          sails.log.verbose(' ##### FaceController:Clean User Face Failed #### ');
