...
var userLimiter = require('../UserLimiter.js');
var canRetry = false;
/**
...
exports.createUser = function (_user, next) {
  ...
  if(!password){
    return next(new SAError({originalError: 'Password can not be null'}));
  }
...
exports.login = function (req, identifier, password, next) {
  ...
  sails.models.user.findOne(query)
  .populate('roles')
        ...
        if(!canRetry){
          passport.validatePassword(password, function (err, res) {
            ...
            if (!res) {
              ...
              canRetry = userLimiter.pwdWrong(user.id);
              return next(null, false);
            } else {
              userLimiter.cleanWrong(user.id);
              canRetry = false;
              return next(null, user, passport);
            }
          });
        }else{
          canRetry = userLimiter.checkStatus(user.id);
          req.flash('error', 'Error.Passport.Password.RetryLimit');
